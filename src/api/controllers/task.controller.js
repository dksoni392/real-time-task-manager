    ...
    try {
        ...
        const task = await task_model_1.Task.create({
            ...
            assignee: taskDto.assigneeId
                ? new mongoose_1.Types.ObjectId(taskDto.assigneeId)
                : undefined,
        });
        // Log the activity
        await (0, activity_service_1.logActivity)(userId, project.team, 'TASK_CREATED', `User ${req.user?.name} created task "${task.title}"`, new mongoose_1.Types.ObjectId(projectId), task._id);
...
const getTasksByProject = async (req, res) => {
    try {
        ...
        const tasks = await task_model_1.Task.find({ project: projectId })
...
const updateTask = async (req, res) => {
    try {
        ...
        Object.keys(taskDto).forEach((key) => taskDto[key] === undefined && delete taskDto[key]);
        ...
        if (taskDto.assigneeId) {
            task.assignee = new mongoose_1.Types.ObjectId(taskDto.assigneeId);
        }
        await task.save();
        // ... (Log activity remains the same) ...
        await (0, activity_service_1.logActivity)(userId, project.team, 'TASK_UPDATED', `User ${req.user.name} updated task "${task.title}"`, project._id, task._id);
...
const deleteTask = async (req, res) => {
    try {
        ...
        await task_model_1.Task.findByIdAndDelete(task._id);
        // 4. Log the activity
        await (0, activity_service_1.logActivity)(userId, project.team, 'TASK_DELETED', `User ${req.user.name} deleted task "${task.title}"`, project._id, task._id);
